#!/bin/bash
	...
	echo "* * * * * * * * * * * * * * * * *$DATE* * * * * * * * * * * * * * * * * * * * *****"
        ...
        vol_id=($(/usr/local/bin/aws ec2 describe-volumes --filters "Name=attachment.status, Values=attached"  --query 'Volumes[].Attachments[].VolumeId' --output text --region $REGION  --profile $PROFILE))
        for i in "${vol_id[@]}"
        do
