...
inst=($(/usr/local/bin/aws ec2 describe-instances --filters "Name=instance-state-name,Values=running"  --query 'Reservations[*].Instances[*].InstanceId' --output text --region $REGION  --profile $PROFILE))
        for i in "${inst[@]}"
        do
